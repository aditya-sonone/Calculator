    ...
    fun postFixConversion(string: String): String{
        ...
        for (s in string){
            if(!notNumeric(s)){
                ...
                while (!st.isEmpty() && operatorPrecedence(s) <= operatorPrecedence(st.peak()!!)){
                    result += " ${st.pop()} "
                }
                st.push(s)
                result += " "
...
class Model{
    ...
    fun result(string: String):String{
        ...
        return try {
            val evaluation = ArithmeticEvaluation().evaluation(postFix).toString()
            //Here
            when{
                evaluation.endsWith(".0") -> evaluation.dropLast(2)
                else -> evaluation
            }
        }catch (e:Exception){
            e.printStackTrace()
...
fun <T> ArrayDeque<T>.peak() = lastOrNull()